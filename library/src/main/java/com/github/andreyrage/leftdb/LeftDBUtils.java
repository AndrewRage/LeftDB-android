...
import static com.github.andreyrage.leftdb.Utils.nullableArrayOfStrings;
import static com.github.andreyrage.leftdb.Utils.nullableString;
...
public abstract class LeftDBUtils implements LeftDBHandler.OnVersionChangeCallback {
	...
	public int countResultsByQuery(SelectQuery query) {
		Cursor cursor = byQuery(db, query);
		if (cursor.moveToFirst()) {
			return cursor.getCount();
		}
		cursor.close();
	...
	private Cursor byQuery(SQLiteDatabase db, SelectQuery query) {
		return db.query(
				query.distinct(),
				query.table(),
